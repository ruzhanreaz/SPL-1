...
{
    if (strstr(expression, "log") != NULL || strstr(expression, "ln") != NULL)
    {
        setposXValue(x);
    }
    else
...
void drawBoldLine(int x1, int y1, int x2, int y2, int thickness)
{
        ...
        for (int j = 0; j < numSamples; j++)
        {
            ...
            if (screenY >= 0 && screenY < SCREEN_HEIGHT)
            {
                ...
                else
                {
                    ...
                    {
                        drawBoldLine(prevScreenX, prevScreenY, screenX, screenY, 3); // Adjust thickness as needed
                    }
                    else
                    {
...
int plot()
{
    ...
    {
        printf("Enter equation Y%d = ", numOfEquations + 1);
        getStrInput(input);
        checkInput(input);
...
void drawALinearEquation(double m, double c)
{
    ...
    double yAtXMin = m * xMin + c;
    double yAtXMax = m * xMax + c;

    bool lineVisible = (yAtXMin >= yMin && yAtXMin <= yMax) ||
                       (yAtXMax >= yMin && yAtXMax <= yMax) ||
                       (yMin >= yAtXMin && yMin <= yAtXMax);

    if (!lineVisible)
    {

        if (fabs(m) < 2.0 && fabs(c) > 10.0)
        {

            double halfRange = (yMax - yMin) / 2;
            yMin = c - halfRange;
            yMax = c + halfRange;
            printf("Window adjusted to center around y-intercept %.2f: y-range [%.1f, %.1f]\n",
                   c, yMin, yMax);
        }
        else
        {
            double padding = (fabs(yAtXMax - yAtXMin) * 0.1) + 2.0;
            yMin = fmin(yAtXMin, yAtXMax) - padding;
            yMax = fmax(yAtXMin, yAtXMax) + padding;
            printf("Window adjusted to show line: y-range [%.1f, %.1f]\n", yMin, yMax);
        }
    }

    if (!validateEquation(equation))
    {
        printf("Warning: Linear equation may still not be visible. Applying more aggressive adjustment.\n");

        double midX = (xMax + xMin) / 2;
        double yAtMidX = m * midX + c;

        double expandedRange = fabs(m) * (xMax - xMin) + fabs(c) + 10.0;
        yMin = yAtMidX - expandedRange / 2;
        yMax = yAtMidX + expandedRange / 2;

        printf("Window adjusted again: y-range [%.1f, %.1f]\n", yMin, yMax);
    }

...
double cov(char *titleName1, char *titleName2)
{
    int index1 = -1;
    int index2 = -1;
    for (int i = 0; i < numOfColumn; i++)
...
void postfixEvaluateStat(Queue *postfix, char *result)
{
    ...
    while (!isQueueEmpty(postfix))
    {
        ...
        if (isFunction(token))
        {
            ...
            else if (strcmp(token, "ln") == 0 || strcmp(token, "log") == 0 || isStatisticalFunction(token))
            {
                char *op = pop(s);
            ...
            else if (strcmp(token, "cov") == 0 || strcmp(token, "corr") == 0)
            {
                char *op2 = pop(s);
                char *op1 = pop(s);
